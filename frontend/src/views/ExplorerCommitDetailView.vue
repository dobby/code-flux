...
const fallbackCommit = computed(() => ({
  repoId: props.repoId,
  commitSha: props.commitSha,
  authorName: 'Eli',
  authoredAt: `${selectedDate.value || dashboard.controls.dateRange.to}T10:12:00Z`,
  subject: 'Refine explorer commit detail',
  linesAdded: 112,
  linesRemoved: 31,
}))
const fallbackFiles = computed(() => [
  { repoId: props.repoId, filePath: 'frontend/src/views/ExplorerView.vue', language: 'Vue', category: 'frontend', subtype: 'view', linesAdded: 42, linesRemoved: 10 },
  { repoId: props.repoId, filePath: 'frontend/src/components/ExplorerAnnotationModal.vue', language: 'Vue', category: 'frontend', subtype: 'component', linesAdded: 38, linesRemoved: 4 },
  { repoId: props.repoId, filePath: 'backend/src/main/kotlin/.../WorkspaceServices.kt', language: 'Kotlin', category: 'backend', subtype: 'service', linesAdded: 32, linesRemoved: 17 },
])

const commit = computed(() => detail.value ?? fallbackCommit.value)
const commitFiles = computed(() => detail.value?.files.slice(0, 20) ?? fallbackFiles.value)
const commitAnnotations = computed(() => detail.value?.annotations ?? [])

async function loadDetail() {
  loading.value = true
  try {
    detail.value = await getExplorerCommitDetail(props.repoId, props.commitSha)
    editorLaunch.value = null
    selectedDate.value = detail.value.authoredAt.slice(0, 10)
  } finally {
    loading.value = false
  }
}
...
function goBack() {
  void router.push({ name: 'explorer', query: selectedDate.value ? { date: selectedDate.value } : undefined })
}
...
watch(
  ...
  ([, date]) => {
    selectedDate.value = typeof date === 'string' ? date : selectedDate.value
    void loadDetail()
  },
)
  ...
  <section class="commit-detail">
    <header class="commit-detail__header">
      ...
      <div class="commit-detail__title">
        <p>Commit detail</p>
        <h1>{{ commit.subject ?? props.commitSha }}</h1>
        <small>{{ props.repoId }} · {{ props.commitSha }}</small>
      </div>
      <button
        class="commit-detail__open"
        type="button"
        :disabled="openingEditor || editorLaunch?.available === false"
    ...
    <div v-else class="commit-detail__grid">
      <article class="commit-detail__panel">
        <section class="commit-detail__summary">
          ...
          <p>{{ commit.authorName }} · {{ selectedDate || 'Unknown day' }}</p>
          <div class="commit-detail__stats">
            <article><span>Files changed</span><strong>{{ commitFiles.length }}</strong></article>
            <article><span>Lines added</span><strong>{{ commit.linesAdded }}</strong></article>
            <article><span>Lines removed</span><strong>{{ commit.linesRemoved }}</strong></article>
          </div>
        </section>
